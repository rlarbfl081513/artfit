...
import UserCustomColor from "../custom/UserCustomColor"
import UserCustomData from "../custom/UserCustomData"
import UserCustomThemu from "../custom/UserCustomThemu"
import LineShowScreen from "./LineShowScreen"
...
function CustomLine() {
            ...
            case 'data':
                ...
                return (
                    ...
                    setDefaultColor1={setDefaultColor1}
                        ...
                        setcarbRatiocolor={setcarbRatiocolor}
                        setproteinRatiocolor={setproteinRatiocolor}
                        setfatRatiocolor={setfatRatiocolor}
    ...
    const [ET, setET] = useState(20); // 운동 시간
    const [BC, setBC] = useState(50); // 아침식단 탄수화물
    const [BP, setBP] = useState(50); // 아침식단 단백질
    const [BF, setBF] = useState(60); // 아침식단 지방
    const [LC, setLC] = useState(120); // 점심식단 탄수화물
    const [LP, setLP] = useState(40);  // 점심식단 단백질
    const [LF, setLF] = useState(50);  // 점심식단 지방
    const [DC, setDC] = useState(120); // 점심식단 탄수화물
    const [DP, setDP] = useState(40);  // 점심식단 단백질
    const [DF, setDF] = useState(50);  // 점심식단 지방
    const [selectedIndices, setSelectedIndices] = useState([]); // Add this state

    const [defaultColor1, setDefaultColor1] = useState([255,239,241,1]);
    const [defaultColor2, setDefaultColor2] = useState([255,191,201, 1]);

    const [carbRatiocolor, setcarbRatiocolor] = useState([255,175,188, 1]);
    const [proteinRatiocolor, setproteinRatiocolor] = useState([255,239,175, 1]);
    const [fatRatiocolor, setfatRatiocolor] = useState([175,255,215, 1]);
    const [watercolor, setwatercolor] = useState([255,166,179, 1]);
    ...
    return (
        <Container>
             <Print>
                ...
                <Modal style={customStyles} isOpen={isSecondModalOpen} onRequestClose={closeSecondModal}>
                    ...
                    <StyledPrintDiv ref={componentRef}>
                        <CardTitle>Line Design_Art Fit</CardTitle>
            ...
            <CustomScreen>
                <CustomMenu>
                    <CustomButton active={activeComponent === 'color'} onClick={() => setActiveComponent('color')}>
                        <CustomColor>Food Palette</CustomColor>
                    </CustomButton>
                    <CustomButton active={activeComponent === 'data'} onClick={() => setActiveComponent('data')}>
                        <CustomData>Exercise & water</CustomData>
                    </CustomButton>
                    <CustomButton active={activeComponent === 'themu'} onClick={() => setActiveComponent('themu')}>
                        <CustomThemu>Color Combination</CustomThemu>
                    </CustomButton>
                </CustomMenu>
           ...
           <Screen>
            <LineShowScreen ET={ET} BC={BC} BP={BP} BF={BF} LC={LC} LP={LP} LF={LF} DC={DC} DP={DP} DF={DF} selectedIndices={selectedIndices}   Default_color_1={defaultColor1}
                    Default_color_2={defaultColor2} carbRatio_color={carbRatiocolor} proteinRatio_color={proteinRatiocolor} fatRatio_color={fatRatiocolor} watercolor={watercolor}
                />
           </Screen>
